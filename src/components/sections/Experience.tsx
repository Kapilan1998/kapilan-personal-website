...
const experiences = [
  {
    ...
    description: [
      "Developed Spring Boot APIs for secure data storage using RSA encryption to Hetzner Cloud S3",
      "Delivered a production client website using Next.js, implementing WEBP optimized image delivery, a multi step interactive quote system and structured SEO metadata",
      "Administered Linux servers and core networking (TCP/IP, DNS, firewall management, traffic routing) across a clustered Proxmox dedicated server environment and Hetzner Cloud VMs",
      "Automated infrastructure management using Ansible for Proxmox VMs/LXC containers and Hetzner Cloud servers",
      "Gained hands on exposure to AWS and foundational experience with Kubernetes deployments",
      "Deployed applications from local development to production using Dokploy PaaS, with Docker containerization and Jenkins CI/CD pipelines",
      "Configured and managed domains/subdomains and traffic routing with Nginx, AdGuard, Cloudflare & pfSense with HAProxy",
      "Deployed Wazuh SIEM and CrowdSec for vulnerability monitoring and intrusion detection, and engineered a zero trust mesh VPN using Headscale and Tailscale",
      "Implemented centralized patch management with PatchMon and monitoring solutions with Checkmk for VM/server health, with real time Google Chat alerting",
